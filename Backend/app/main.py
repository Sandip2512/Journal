...
from app.routes import mt5  # Import mt5 router
from app.routes import leaderboard  # Import leaderboard router
...
# Middleware to ensure headers on all responses and log requests


# Handle OPTIONS preflight requests - catch all
...
# Create DB tables (also done in startup, but keeping for compatibility)
Base.metadata.create_all(bind=engine)
...
def get_mt5_connection_status(user_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if recent_trades and len(recent_trades) > 0:
            # Get the most recent trade's creation time
            from app.models.trade import Trade
            most_recent = db.query(Trade).filter(
                Trade.user_id == user_id
            ).order_by(Trade.open_time.desc()).first()
            if most_recent:
                last_fetch = most_recent.open_time.isoformat() if hasattr(most_recent.open_time, 'isoformat') else str(most_recent.open_time)
...
def debug_user(user_id: str, db: Session = Depends(get_db)):
    user = get_user_by_id(db, user_id)
    if user:
        return {
            "exists": True, 
...
def debug_mt5_credentials(user_id: str, db: Session = Depends(get_db)):
    credentials = get_mt5_credentials(db, user_id)
    if credentials:
        return {
            "exists": True,
...
# ----------------- MT5 Routes -----------------
from app.routes import auth, admin, admin_users, admin_trades, admin_system, admin_analytics, announcements
...
def add_mt5_credentials(user_id: str, credentials: MT5CredentialsCreate, db: Session = Depends(get_db)):
    user = get_user_by_id(db, user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def get_user_mt5_credentials(user_id: str, db: Session = Depends(get_db)):
    credentials = get_mt5_credentials(db, user_id)
    if not credentials:
...
def update_user_mt5_credentials(user_id: str, credentials: MT5CredentialsCreate, db: Session = Depends(get_db)):
...
def delete_user_mt5_credentials(user_id: str, db: Session = Depends(get_db)):
...
def get_trades_by_user(user_id: str, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def get_trade_by_number(trade_no: int, db: Session = Depends(get_db)):
...
def update_trade_reason_mistake(trade_no: int, reason: str, mistake: str, db: Session = Depends(get_db)):
...
def delete_trade_by_number(trade_no: int, db: Session = Depends(get_db)):
    trade = get_trade_by_trade_no(db, trade_no)
    if not trade:
        raise HTTPException(status_code=404, detail="Trade not found")
...
def fetch_mt5_trades_endpoint(user_id: str, db: Session = Depends(get_db)):
    try:
        ...
        if not user:
            try:
                ...
                if user:
                    print(f"✅ Found user by account number: {account_number} -> {user.user_id}")
                    user_id = user.user_id  # Update user_id to the actual UUID
            except ValueError:
        ...
        try:
            account_number = int(credentials.account)
        except (ValueError, TypeError) as e:
            raise HTTPException(status_code=400, detail=f"Invalid MT5 account number: {credentials.account}")
...
def get_trade_statistics(user_id: str, db: Session = Depends(get_db)):
    trades = get_trades(db, user_id, 0, 1000)
    if not trades:
        return {"message": "No trades found", "user_id": user_id}
    
    total_trades = len(trades)
    total_profit = sum(trade.profit_amount for trade in trades)
    total_loss = sum(trade.loss_amount for trade in trades)
